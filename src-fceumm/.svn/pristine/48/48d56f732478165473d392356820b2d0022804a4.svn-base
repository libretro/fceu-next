...
//    ROM_size = head.ROM_size;
  ...
  */
  ...
  else
  {
   ...
   SetupCartPRGMapping(1,WRAM,8192,1); // батареечная память для старого iNES, в новом iNES каждый
                                       // маппер должен сам следить за своим WRAM/SRAM.
                                       // решить, надо ли подключать SRAM автоматически, если стоит
                                       // бит в заголовке, или все равно отдавать это мапперу, и как
                                       // тогда решать вопрос с остальными мапперами с потенциально
                                       // возможным срам битом. определить все мапперы, которые могут
                                       // иметь батарейку и вообще не проверяя бит всегда их сохранять
   if(head.ROM_type&2)
   {
...
static void iNESPower(void)
{
  ...
  /* This statement represents atrocious code.  I need to rewrite
    ...
    if(type==19 || type==6 || type==69 || type==85)
...
//    {218,  Mapper218_Init},
    {219, UNLA9746_Init},
    {220, UNLKS7057_Init}, // DEBUG mapper
    {221, UNLN625092_Init},
    {222, Mapper222_Init},
//    {223,  Mapper223_Init},
//    {224,  Mapper224_Init},
