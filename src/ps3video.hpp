...
#include "emu-ps3-constants.h"
	...
	#define CELL_PRINTF(fmt, args...) do {\
	   gl_dprintf(0.1, 0.1, 0.90, fmt, ##args);\
...
class PS3Graphics
{
	public:
		...
		PS3Graphics(uint32_t resolution, uint32_t aspect, uint32_t smooth, uint32_t smooth2, const char * shader, const char * shader2, uint32_t overscan, float overscan_amount, uint32_t pal60Hz, uint32_t vsync, uint32_t tripleBuffering, uint32_t viewport_x, uint32_t viewport_y, uint32_t viewport_width, uint32_t viewport_height); 
		~PS3Graphics();
		...
		void UpdateCgParams(unsigned width, unsigned height, unsigned tex_width, unsigned tex_height, unsigned view_width, unsigned view_height, unsigned index = 0);
		void ResetFrameCounter();
		void resize_aspect_mode_input_loop(uint64_t state);

		/* calculate functions */
		int calculate_aspect_ratio_before_game_load(void);
		...
		/* libdbgfont */
		void InitDbgFont();
		void dprintf_noswap(float, float, float, const char*, ...);

		/* resolution functions */

		void ChangeResolution(uint32_t resId, uint32_t pal60Hz, uint32_t tripleBuffering, uint32_t scaleEnabled, uint32_t scaleFactor);
		void SwitchResolution(uint32_t resId, uint32_t pal60Hz, uint32_t tripleBuffering, uint32_t scaleEnabled, uint32_t scaleFactor);
		void NextResolution();
	...
	private:
		...
		uint32_t m_currentResolutionPos;
		int m_resolutionId;
		uint32_t fbo_scale;
		...
		void setup_texture(GLuint texture, unsigned width, unsigned height);
		
		/* PSGL */
		int32_t PSGLInit(uint32_t scaleEnable, uint32_t scaleFactor);
		void PSGLInitDevice(uint32_t resolutionId = 0, uint32_t pal60Hz = 0, uint32_t tripleBuffering = 1);
