...
static void set_setting_label(menu * menu_obj, int currentsetting)
{
	switch(currentsetting)
	{
		...
		case SETTING_FCEU_GAME_GENIE:
			if(Settings.FCEUGameGenie)
			{
				menu_obj->items[currentsetting].text_color = GREEN;
				snprintf(menu_obj->items[currentsetting].setting_text, sizeof(menu_obj->items[currentsetting].setting_text), "ON");
				snprintf(menu_obj->items[currentsetting].comment, sizeof(menu_obj->items[currentsetting].comment), "INFO - [Game Genie] is set to 'ON'.\nNOTE: Game Genie ROM must be called 'gg.rom' and has to be in the USRDIR directory.");
			}
			else
			{
				menu_obj->items[currentsetting].text_color = ORANGE;
				snprintf(menu_obj->items[currentsetting].setting_text, sizeof(menu_obj->items[currentsetting].setting_text), "OFF");
				snprintf(menu_obj->items[currentsetting].comment, sizeof(menu_obj->items[currentsetting].comment), "INFO - [Game Genie] is set to 'OFF'.");
			}
			break;
		...
		case SETTING_FCEU_PALETTE:
			if(!Settings.FCEUPalette)
				menu_obj->items[currentsetting].text_color = GREEN;
			else
				menu_obj->items[currentsetting].text_color = ORANGE;

			snprintf(menu_obj->items[currentsetting].setting_text, sizeof(menu_obj->items[currentsetting].setting_text), "#%d: %s", Settings.FCEUPalette, palettes[Settings.FCEUPalette].desc);
			snprintf(menu_obj->items[currentsetting].comment, sizeof(menu_obj->items[currentsetting].comment), "INFO - Select a color palette from the list.");
			break;
		case SETTING_FCEU_DISABLE_SPRITE_LIMIT:
			if(Settings.FCEUDisableSpriteLimitation)
			{
				menu_obj->items[currentsetting].text_color = GREEN;
				snprintf(menu_obj->items[currentsetting].setting_text, sizeof(menu_obj->items[currentsetting].setting_text), "ON");
				snprintf(menu_obj->items[currentsetting].comment, sizeof(menu_obj->items[currentsetting].comment), "INFO - [Sprite Limit] is set to 'ON'.");
			}
			else
			{
				menu_obj->items[currentsetting].text_color = ORANGE;
				snprintf(menu_obj->items[currentsetting].setting_text, sizeof(menu_obj->items[currentsetting].setting_text), "OFF");
				snprintf(menu_obj->items[currentsetting].comment, sizeof(menu_obj->items[currentsetting].comment), "INFO - [Sprite Limit] is set to 'OFF'. Less sprite flickering will be\nnoticeabe.");
			}
			break;
		case SETTING_FCEU_PPU_MODE:
			if(Settings.FCEUPPUMode)
			{
				menu_obj->items[currentsetting].text_color = GREEN;
				snprintf(menu_obj->items[currentsetting].setting_text, sizeof(menu_obj->items[currentsetting].setting_text), "New");
				snprintf(menu_obj->items[currentsetting].comment, sizeof(menu_obj->items[currentsetting].comment), "INFO - [PPU Mode] is set to 'New'. Graphis are more accurate\nat some speed loss.");
			}
			else
			{
				menu_obj->items[currentsetting].text_color = ORANGE;
				snprintf(menu_obj->items[currentsetting].setting_text, sizeof(menu_obj->items[currentsetting].setting_text), "Old");
				snprintf(menu_obj->items[currentsetting].comment, sizeof(menu_obj->items[currentsetting].comment), "INFO - [PPU Mode] is set to 'Old'. Less accurate graphics but faster\noverall.");
			}
			break;
		...
		case SETTING_PATH_BASE_DIRECTORY:
			if(!(strcmp(Settings.PS3PathBaseDirectory, usrDirPath)))
				menu_obj->items[currentsetting].text_color = GREEN;
			else
				menu_obj->items[currentsetting].text_color = ORANGE;

			snprintf(menu_obj->items[currentsetting].setting_text, sizeof(menu_obj->items[currentsetting].setting_text), Settings.PS3PathBaseDirectory);
			break;
